...
describe('test overpass url generation', () => {
    ...
    test('build URL with 2 param', () => {
        ...
        const expected_response = 'http://overpass-api.de/api/interpreter?data=[out:json][timeout:25][bbox:45.7073666,4.7718134,45.8082628,4.8983774];node[amenity=pub];out;node[amenity=restaurant];out;';
        const response = sc.overpassURL(cp, filters);
        expect(response).toBe(expected_response);
    });
});
